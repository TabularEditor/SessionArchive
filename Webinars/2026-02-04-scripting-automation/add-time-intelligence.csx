...
Action<string, string> AddCalc = (s, t) => {
    ...
    checks.Add(checkBox);
    var textBox = new TextBox { Location = new Point(350,0), Width = 200, Text = t };
    suffixes.Add(textBox);
    panel.Controls.Add(textBox);
    flowPanel.Controls.Add(panel);
};
AddLabel("Selection: " + Selected.Summary());
...
Action CheckCalc = () => {
    ...
    if (calendar == null) {
        ...
        checks[0].Enabled = calendar.GetTimeUnits().Any(t => t.TimeUnit is TimeUnit.Week or TimeUnit.WeekOfMonth or TimeUnit.WeekOfYear or TimeUnit.WeekOfQuarter);
        checks[0].Checked = checks[0].Enabled;
        checks[1].Enabled = calendar.GetTimeUnits().Any(t => t.TimeUnit is TimeUnit.Month or TimeUnit.MonthOfQuarter or TimeUnit.MonthOfYear);
        checks[1].Checked = checks[1].Enabled;
